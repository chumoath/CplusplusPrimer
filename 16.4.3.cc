...
template<typename... Args> /* 函数模板参数列表 */
/* 1. 模板参数包 按 右值引用的模式扩展 (A1 && a1, A2 && a2, ..., An && an)*/
void Vec<T>::emplace_back(Args&&... args){
    check_n_alloc();
    /*  2. 所有参数都进行转发
            std::forward<A1>(a1), std::forward<A2>(a2), ..., std::forward<An>(an)
     */
...
void test2(){
    ...
    /* 应该调用 string 的 移动构造函数 */
    svec.emplace_back(std::move(s));
    for(auto & s : svec)
        cout << s << endl;
}
